...
private:
	...
	bool LoadPage(const std::string & pagename, const std::string & path, const std::string & texpath, const std::string & datapath, CONFIGFILE & carcontrolsfile, SCENENODE * s, std::map <std::string, FONT> & fonts, std::map<std::string, GUIOPTION> & optionmap, float screenhwratio, const std::string & texsize, std::ostream & error_output)
	{
		...
		if (!p.page.Load(path+"/"+pagename, texpath, datapath, carcontrolsfile, p.node, textures, fonts, optionmap, screenhwratio, texsize, error_output)) return false;
...
public:
	GUI() : node(NULL),animation_counter(0),animation_count_start(0),syncme(false),syncme_from_external(false),control_load(false),ingame(false) {active_page = last_active_page = pages.end();}
	...
	bool Load(const std::list <std::string> & pagelist, const std::map<std::string, 
		...
		const std::string & datapath, SCENENODE * s, std::map <std::string, FONT> & fonts, 
	...
	void ActivatePage(const std::string & pagename, float activation_time, std::ostream & error_output, bool save_options=false)
	{
		...
		if (active_page != pages.end() && ok) active_page->second.page.UpdateOptions(true, optionmap, error_output);
		...
		pages[pagename].page.SetVisible(true);
					     ...
					     bool cursordown, bool cursorjustup, float screenhwratio, std::ostream & error_output)
	{
		...
		if (active_page != pages.end())
		{
			actions = active_page->second.page.ProcessInput(movedown, moveup, cursorx, cursory,
		...
		for (std::list <std::pair <std::string, bool> >::iterator i = actions.begin(); i != actions.end(); ++i)
		{
			...
			else
			{
				...
				if (i->second)
				{
					...
					active_page->second.page.UpdateOptions(true, optionmap, error_output);
	...
	void Update(float dt)
	{
		...
		if (active_page != pages.end())
		{
			...
			active_page->second.page.SetAlpha(3*p*p-2*p*p*p);
		}
		...
		if (last_active_page != pages.end())
		{
			if (animation_counter > 0)
			{
				...
				last_active_page->second.page.SetAlpha(3*p*p-2*p*p*p);
			}
			else
			{
				last_active_page->second.page.SetVisible(false);
		...
		if (active_page != pages.end())
			active_page->second.page.Update(dt);
		if (last_active_page != pages.end())
			last_active_page->second.page.Update(dt);
	}
