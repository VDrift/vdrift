	...
	CarTireState & s) const
{
	...
	btScalar sin_camber = Clamp(s.camber, btScalar(-0.3), btScalar(0.3)); // +-18 deg
	...
	btScalar wr = std::copysign(Max(std::abs(rot_velocity), btScalar(1E-12)), rot_velocity);
	btScalar rwr = 1 / wr;
	btScalar sx = -vrx * rwr;
	btScalar sy = -vry * rwr;
	...
	btScalar * pmz) const
{
	...
	if (pfy)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			uc = Min(0.5f * (t2 - t0 - 5), btScalar(1));
			ud = uc + 1;
			btScalar ue = uc - 1;
			btScalar uf = 3 * uc + 5;
...
void CarTire3::findIdealSlip(btScalar fz, btScalar slip[2]) const
{
	...
	while (true)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			btScalar t1 = t0 + 3;
			btScalar t2 = btSqrt(t1 * t1 + rp4);
			btScalar uc = Min(0.5f * (t2 - t0 - 5), btScalar(1));
			btScalar ud = uc + 1;
			btScalar ue = uc - 1;
			btScalar uf = 3 * uc + 5;
