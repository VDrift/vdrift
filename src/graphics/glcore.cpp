...
int GLC_ARB_draw_elements_base_vertex = GLC_LOAD_FAILED;
...
int GLC_ARB_texture_rectangle = GLC_LOAD_FAILED;
int GLC_ARB_multisample = GLC_LOAD_FAILED;
...
typedef struct glcStrToExtMap_s
{
...
static glcStrToExtMap ExtensionMap[10] = {
	...
	{"GL_ARB_draw_elements_base_vertex", &GLC_ARB_draw_elements_base_vertex, NULL},
	...
	{"GL_ARB_texture_rectangle", &GLC_ARB_texture_rectangle, NULL},
	{"GL_ARB_multisample", &GLC_ARB_multisample, Load_ARB_multisample},
};
...
{
	int loop;
	glcStrToExtMap *currLoc = ExtensionMap;
...
static void ClearExtensionVars(void)
{
	...
	GLC_ARB_draw_elements_base_vertex = GLC_LOAD_FAILED;
	...
	GLC_ARB_texture_rectangle = GLC_LOAD_FAILED;
	GLC_ARB_multisample = GLC_LOAD_FAILED;
}

static void EnableExtensionsInCore()
{
	int loop;
	for(loop = g_extensionMapSizeCore; loop < g_extensionMapSize; ++loop)
	{
		*(ExtensionMap[loop].extensionVariable) = GLC_LOAD_SUCCEEDED;
	}
}

static void LoadExtByName(const char *extensionName, int extensionMapSize)
{
	glcStrToExtMap *entry = NULL;
	entry = FindExtEntry(extensionName, extensionMapSize);
	if(entry)
	{
...
static void ProcExtsFromExtList(void)
{
	...
	for(iLoop = 0; iLoop < iNumExtensions; iLoop++)
	{
		...
		LoadExtByName(strExtensionName, g_extensionMapSizeCore);
	}
}
...
static void ProcExtsFromExtString(const char *strExtList)
{
	...
	while(*strCurrPos)
	{
		...
		LoadExtByName(strWorkBuff, g_extensionMapSize);
...
int glcLoadFunctions(void)
{
	int numFailed = 0;
	ClearExtensionVars();

	_ptrc_glGetString = (const GLubyte * (CODEGEN_FUNCPTR *)(GLenum))IntGetProcAddress("glGetString");
	if(!_ptrc_glGetString) return GLC_LOAD_FAILED;

	_ptrc_glGetIntegerv = (void (CODEGEN_FUNCPTR *)(GLenum, GLint *))IntGetProcAddress("glGetIntegerv");
	if(!_ptrc_glGetIntegerv) return GLC_LOAD_FAILED;

	ParseVersionFromString(&g_major_version, &g_minor_version, (const char*)_ptrc_glGetString(GL_VERSION));
	if(g_major_version > 2)
	{
		_ptrc_glGetStringi = (const GLubyte * (CODEGEN_FUNCPTR *)(GLenum, GLuint))IntGetProcAddress("glGetStringi");
		if(!_ptrc_glGetStringi) return GLC_LOAD_FAILED;

		/*Better safe than sorry*/
		_ptrc_glGetIntegerv(GL_MAJOR_VERSION, &g_major_version);
		_ptrc_glGetIntegerv(GL_MINOR_VERSION, &g_minor_version);

		ProcExtsFromExtList();

		EnableExtensionsInCore();
	}
	else
	{
...
const GLubyte * glcErrorString(GLenum error)
{
	struct Error
	{
		GLenum code;
