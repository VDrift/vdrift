...
bool Renderer::initialize(const std::vector <RealtimeExportPassInfo> & config, StringIdMap & stringMap, const std::string & shaderPath, unsigned int w, unsigned int h, std::ostream & errorOutput)
{
	...
	for (std::vector <RealtimeExportPassInfo>::const_iterator i = config.begin(); i != config.end(); i++,passCount++)
	{
		...
		if (shaders.find(vertexShaderName) == shaders.end())
			if (!loadShader(shaderPath.empty() ? i->vertexShader : shaderPath+"/"+i->vertexShader, vertexShaderName, i->vertexShaderDefines, GL_VERTEX_SHADER, errorOutput))
				return false;
		if (shaders.find(fragmentShaderName) == shaders.end())
			if (!loadShader(shaderPath.empty() ? i->fragmentShader : shaderPath+"/"+i->fragmentShader, fragmentShaderName, i->fragmentShaderDefines, GL_FRAGMENT_SHADER, errorOutput))
				return false;
